...
$query="select * from house h join rating r on h.house_id=r.house_id ";
$data=mysqli_query($conn,$query);
while($result=mysqli_fetch_assoc($data))
{
 echo "<tr><td>".$result['house_id']."</td><td>".$result['no_of_rooms']."</td><td> ".$result['address']."</td><td>";
echo  $result['city']."</td><td>".$result['state']."</td><td>".$result['country']."</td><td> ".$result['description']."</td><td>".$result['rent']."</td><td>";
echo '<img src="data:pics/jpeg;base64,'.base64_encode( $result['house_picture'] ).'"/>';
